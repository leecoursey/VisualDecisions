...
(function($){
    ...
    function render(){
        // links with labels
        var linkSel = svg.selectAll('g.link').data(data.links);
        var linkEnter = linkSel.enter().append('g').attr('class','link');
        linkEnter.append('line').attr('stroke', '#000');
        ...
        linkSel.select('text')
            .attr('x', function(d){ var s=findNode(d.source); var t=findNode(d.target); return (s.x+t.x)/2; })
            .attr('y', function(d){ var s=findNode(d.source); var t=findNode(d.target); return (s.y+t.y)/2; })
        ...
        nodeSel.on('click', function(event,d){
            if(selected && selected !== d){
                ...
                data.links.push({source:selected.id, target:d.id, label: label || ''});
                selected = null;
                render();
                save();
            } else {
